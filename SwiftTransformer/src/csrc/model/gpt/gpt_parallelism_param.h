...
struct GptParallelismParam {
    ...
    {
        if (parallel_config.size() > 4) {
            if (parallel_config.size() != (4 + pipeline_para_size)) {
                throw std::invalid_argument("parallel_config size does not fit pipeline parallelism size.");
            }
            local_layer_num = parallel_config[4 + pipeline_para_rank];
            for (int i = 0; i < pipeline_para_rank; ++i) {
                layer_begin += parallel_config[4 + i];
            }
