...
def convert_weights(
    ...
    for file in input_files:
        ...
        state_dict.update(torch.load(file, torch.device("cpu"), weights_only=True))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dtype", "-d", type=str, required=True, help="Data type of the weights")
    ...
    if args.dtype not in dtype_map:
        raise ValueError(f"Unsupported dtype {args.dtype}. Supported dtypes are {list(dtype_map.keys())}")
    convert_weights(args.input, args.output, dtype_map[args.dtype], args.model)